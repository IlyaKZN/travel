...
import { defaultAvatar, defaultTripImage } from '@/utils/unsplash'
...
<template>
  <RouterLink :to="`/trips/${trip.id}`" class="card block overflow-hidden transition hover:shadow-md">
    <img
      :src="trip.images[0] || defaultTripImage()"
      :alt="trip.location"
      class="aspect-[16/9] w-full object-cover"
    />
    <div class="p-4">
      <div class="mb-3 flex items-center gap-2">
        <img :src="trip.creatorAvatar || defaultAvatar()" :alt="trip.creatorName" class="h-8 w-8 rounded-full object-cover" />
      ...
      <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
        ...
        <span class="rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-1">{{ formatSpots(trip.freeSpots, trip.maxParticipants) }}</span>
      </div>
