...
import {
  ...
  Calendar, Clock, Users, Check, Send,
  ChevronLeft, ChevronRight, Bell,
  Home, Plus, User, MessageSquare,
  Navigation, ArrowUpDown, Camera, Settings,
  Zap, X, Filter,
  TrendingUp,
} from "lucide-react";
import { TRIPS, MESSAGES, DM_CONVERSATIONS, DM_MESSAGES, type Trip, type Message, type DmConversation } from "../lib/mockData";
import {
  transportLabel,
  seatsLeftLabel,
  participantsLabel,
...
import { getTripAvatarColors, getTripAvatarLetters } from "../lib/tripAvatar";

// ─── Types ───────────────────────────────────────────────────────────────────

type Screen = "onboarding" | "home" | "detail" | "create" | "chat" | "tripChat" | "dmChat" | "profile";
type Tab = "home" | "create" | "chat" | "profile";
type TransportType = "car" | "train" | "bus" | "plane";
type ChatView = "trip" | "personal";
const PARTICIPANTS_LIMIT = 10;
const DESKTOP_BREAKPOINT = 1024;

function tripTotalParticipants(trip: Trip) {
  return trip.takenSeats + 1;
}

function tripMaxParticipants(trip: Trip) {
  return trip.seats + 1;
}
...
function TripAvatar({ trip, className = "" }: { trip: Trip; className?: string }) {
  const { bg, text } = getTripAvatarColors(trip.from, trip.to);
  const letters = getTripAvatarLetters(trip.fromShort, trip.toShort);
  return (
    <div className={`flex items-center justify-center font-extrabold tracking-tight ${className}`} style={{ backgroundColor: bg, color: text }}>
      {letters}
    </div>
  );
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-7 pb-8 bg-background">
        ...
        <div className="space-y-2.5 mb-7">
          {[
            { icon: <Zap size={15} />, text: "Проверенные профили участников" },
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-gray-500 font-medium">
            ...
            <span>{participantsLabel(totalParticipants, maxParticipants)}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-24 space-y-4" style={{ scrollbarWidth: "none" }}>
        ...
        {filteredTrips.length === 0 ? (<div className="flex flex-col items-center py-12 text-gray-400"><MapPin size={40} className="mb-3 text-orange-200" /><p className="text-sm font-medium">Поездок по этому фильтру не найдено</p></div>) : (filteredTrips.map((trip) => <TripCard key={trip.id} trip={trip} onSelect={() => onTripSelect(trip)} />))}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        <div className="bg-white rounded-t-[2rem] -mt-4 px-5 pt-5 pb-6">
          <div className="grid grid-cols-4 gap-2 mb-5">
            {[
              { icon: <Calendar size={14} className="text-orange-500" />, label: trip.date.split(",")[0]?.trim() ?? trip.date, sub: "Начало" },
              { icon: <Clock size={14} className="text-orange-500" />, label: trip.time, sub: "Отправление" },
              { icon: <Calendar size={14} className="text-orange-500" />, label: trip.endDate.split(",")[0]?.trim() ?? trip.endDate, sub: "Окончание" },
              { icon: <Clock size={14} className="text-orange-500" />, label: trip.endTime, sub: "Прибытие" },
            ].map((item, i) => (
          ...
          <div className="grid grid-cols-2 gap-2 mb-5">
            {[
              ...
              { icon: <Users size={14} className="text-orange-500" />, label: `${totalParticipants}`, sub: "Участников" },
            ].map((item, i) => (
              <div key={i} className="flex flex-col items-center gap-1 bg-orange-50 rounded-2xl py-3 px-1">{item.icon}<span className="text-xs font-extrabold text-gray-800 text-center leading-tight">{item.label}</span><span className="text-[9px] text-gray-400 text-center">{item.sub}</span></div>
            ))}
          </div>
          <div className="flex items-center gap-3 mb-5 p-3.5 bg-orange-50 rounded-2xl">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Организатор поездки</p>
            </div>
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-3"><h3 className="font-extrabold text-gray-900 text-sm">Участники ({totalParticipants})</h3><span className="text-xs text-orange-500 font-bold">{seatsLeft} свободных мест</span></div>
      ...
      <div className="px-5 pb-8 pt-3 bg-white border-t border-orange-50">
        <button onClick={onJoin} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-extrabold text-base active:scale-95 transition-transform" style={{ boxShadow: "0 8px 24px rgba(249,115,22,0.35)" }}>Запросить участие</button>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-28 space-y-4" style={{ scrollbarWidth: "none" }}>
        ...
        <div className="bg-white rounded-3xl p-4 border border-orange-50" style={{ boxShadow: "0 2px 12px rgba(249,115,22,0.07)" }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setSeats(Math.min(PARTICIPANTS_LIMIT, seats + 1))} className="w-12 h-12 rounded-2xl bg-orange-500 text-white flex items-center justify-center font-extrabold text-2xl" style={{ boxShadow: "0 4px 12px rgba(249,115,22,0.35)" }}>+</button>
          </div>
      ...
      <div className="px-5 pt-14 pb-4">
        ...
        <div className="flex gap-2">
          {([{ id: "trip" as const, label: "Поездки" }, { id: "personal" as const, label: "Личные" }]).map((tab) => (
            <button key={tab.id} onClick={() => setActiveView(tab.id)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeView === tab.id ? "bg-orange-500 text-white" : "bg-white text-gray-600 border border-gray-100"}`}>{tab.label}</button>
          ))}
        </div>
      </div>
      <div className="flex-1 overflow-y-auto px-5 pb-24 space-y-2" style={{ scrollbarWidth: "none" }}>
        {activeView === "trip" ? TRIPS.map((t, i) => (
          <button key={t.id} onClick={() => onTripChat(t)} className="w-full flex items-center gap-3 bg-white rounded-2xl p-3.5 border border-orange-50 text-left" style={{ boxShadow: "0 1px 8px rgba(249,115,22,0.06)" }}>
            <TripAvatar trip={t} className="w-12 h-12 rounded-2xl text-sm flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-gray-800 truncate">{t.fromShort} → {t.toShort}</p>
              <p className="text-xs text-gray-400 truncate">{t.date} · групповой чат</p>
            </div>
          ...
          <button key={dm.id} onClick={() => onDmChat(dm)} className="w-full flex items-center gap-3 bg-white rounded-2xl p-3.5 border border-orange-50 text-left" style={{ boxShadow: "0 1px 8px rgba(249,115,22,0.06)" }}>
            <img src={dm.user.avatar} alt={dm.user.name} className="w-12 h-12 rounded-2xl object-cover flex-shrink-0" />
            <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-col items-end gap-1 flex-shrink-0">
              ...
              {dm.unread ? <div className="w-5 h-5 bg-orange-500 rounded-full flex items-center justify-center"><span className="text-[9px] font-extrabold text-white">{dm.unread}</span></div> : null}
            </div>
          </button>
        ))}
      </div>
...
function DmChatScreen({ dm, onBack }: { dm: DmConversation; onBack: () => void }) {
...
function DesktopHome({ onTripSelect }: { onTripSelect: (t: Trip) => void; onCreateClick: () => void }) {
  ...
  const hasActiveFilters = selectedTransports.length > 0 || minSeats > 1 || activeChip !== "all";
        ...
        <div className="grid grid-cols-3 gap-8 items-start">
          <div className="col-span-2 space-y-5">
            <div className="relative h-72 rounded-3xl overflow-hidden">
              <TripAvatar trip={trip} className="w-full h-full text-6xl" />
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                { icon: <Calendar size={18} className="text-orange-500" />, label: trip.date, sub: "Начало" },
                { icon: <Clock size={18} className="text-orange-500" />, label: trip.time, sub: "Отправление" },
                { icon: <Calendar size={18} className="text-orange-500" />, label: trip.endDate, sub: "Окончание" },
                { icon: <Clock size={18} className="text-orange-500" />, label: trip.endTime, sub: "Прибытие" },
              ].map((item, i) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { icon: <Users size={18} className="text-orange-500" />, label: participantsLabel(totalParticipants, tripMaxParticipants(trip)), sub: "Участники" },
              ].map((item, i) => (
                <div key={i} className="flex flex-col items-center gap-2 bg-orange-50 rounded-2xl py-4 px-3">
                  {item.icon}<span className="text-sm font-extrabold text-gray-800 text-center">{item.label}</span><span className="text-xs text-gray-400 text-center">{item.sub}</span>
                </div>
            ...
            <div className="bg-white rounded-3xl p-5 border border-orange-50" style={{ boxShadow: "0 2px 12px rgba(249,115,22,0.07)" }}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">Организатор поездки</p>
                </div>
            ...
            <div className="bg-white rounded-3xl p-5 border border-orange-50" style={{ boxShadow: "0 2px 12px rgba(249,115,22,0.07)" }}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-extrabold text-gray-900 text-base">Участники ({totalParticipants})</h3>
          ...
          <div className="col-span-1">
            <div className="sticky top-4 bg-white rounded-3xl p-5 border border-orange-50 space-y-4" style={{ boxShadow: "0 4px 24px rgba(249,115,22,0.12)" }}>
              <div>
                <div className="text-lg font-extrabold text-gray-900">Информация</div>
                <p className="text-sm text-gray-400 mt-0.5">Детали поездки и участие</p>
              </div>
              <div className="bg-orange-50 rounded-2xl p-4 space-y-2.5">
                {[
                  ...
                  { icon: <Calendar size={13} className="text-orange-500" />, label: "Начало", value: `${trip.date}, ${trip.time}` },
                  { icon: <Calendar size={13} className="text-orange-500" />, label: "Окончание", value: `${trip.endDate}, ${trip.endTime}` },
                  { icon: <Users size={13} className="text-orange-500" />, label: "Участники", value: participantsLabel(totalParticipants, tripMaxParticipants(trip)) },
                  { icon: <Users size={13} className="text-orange-500" />, label: "Свободно", value: `${seatsLeft} из ${trip.seats}` },
                ].map((row, i) => (
                  <div key={i} className="flex items-center gap-2">{row.icon}<span className="text-xs text-gray-400 w-20 flex-shrink-0">{row.label}</span><span className="text-sm font-bold text-gray-800 ml-auto text-right">{row.value}</span></div>
                ))}
              </div>
              ...
              <div className="border-t border-orange-50 pt-4 flex items-center gap-3">
                ...
                <div><p className="text-sm font-bold text-gray-900">{trip.host.name}</p><p className="text-xs text-gray-400">Организатор</p></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl p-4 border border-orange-50" style={{ boxShadow: "0 1px 8px rgba(249,115,22,0.06)" }}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setSeats(Math.min(PARTICIPANTS_LIMIT, seats + 1))} className="w-10 h-10 rounded-xl bg-orange-500 text-white flex items-center justify-center font-extrabold text-xl" style={{ boxShadow: "0 4px 8px rgba(249,115,22,0.35)" }}>+</button>
                </div>
...
function DesktopChat({ trip, selectedDm, onSelectTrip, onSelectDm }: {
  trip: Trip;
  selectedDm: DmConversation | null;
  onSelectTrip: (t: Trip) => void;
  onSelectDm: (dm: DmConversation) => void;
}) {
  const [chatView, setChatView] = useState<ChatView>(selectedDm ? "personal" : "trip");
  const [messages, setMessages] = useState<Message[]>(selectedDm ? DM_MESSAGES : MESSAGES);
  const [input, setInput] = useState("");
  const bottomRef = useRef<HTMLDivElement>(null);
  const chatMsgsRef = useRef<HTMLDivElement>(null);
  const activeDm = selectedDm ?? DM_CONVERSATIONS[0];
  const activeTrip = trip;
  const totalParticipants = tripTotalParticipants(activeTrip);

  useEffect(() => {
    setMessages(chatView === "personal" ? DM_MESSAGES : MESSAGES);
  }, [chatView, activeDm.id, activeTrip.id]);
    ...
    <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
          {chatView === "trip" ? TRIPS.map((t, i) => (
            ...
            <button key={dm.id} onClick={() => { onSelectDm(dm); setChatView("personal"); }} className={`w-full flex items-center gap-3 px-4 py-3.5 border-b border-orange-50 transition-colors text-left ${dm.id === activeDm.id ? "bg-orange-50" : "hover:bg-background"}`}>
              ...
              {dm.unread ? <div className="w-5 h-5 bg-orange-500 rounded-full flex items-center justify-center flex-shrink-0"><span className="text-[9px] font-extrabold text-white">{dm.unread}</span></div> : null}
            </button>
          ))}
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-3.5 bg-white border-b border-orange-50 flex-shrink-0">
          {chatView === "personal" ? (
            <>
              <img src={activeDm.user.avatar} alt={activeDm.user.name} className="w-10 h-10 rounded-2xl object-cover flex-shrink-0" />
              <div><p className="font-extrabold text-gray-900">{activeDm.user.name}</p><p className="text-xs text-gray-400">Личные сообщения</p></div>
            </>
          ) : (
            <>
              <TripAvatar trip={activeTrip} className="w-10 h-10 rounded-2xl text-xs flex-shrink-0" />
              <div><p className="font-extrabold text-gray-900">{activeTrip.fromShort} → {activeTrip.toShort}</p><p className="text-xs text-gray-400">{participantsLabel(totalParticipants)} · {activeTrip.date}</p></div>
            </>
          )}
        </div>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex items-end gap-2 ${msg.isMe ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`max-w-[65%] flex flex-col gap-0.5 ${msg.isMe ? "items-end" : "items-start"}`}>
                {chatView === "trip" && !msg.isMe && <span className="text-xs text-gray-400 font-medium ml-1">{msg.sender}</span>}
                <div className={`px-4 py-2.5 text-sm leading-relaxed font-medium ${msg.isMe ? "bg-orange-500 text-white rounded-2xl rounded-br-sm" : "bg-white text-gray-800 rounded-2xl rounded-bl-sm"}`} style={!msg.isMe ? { boxShadow: "0 1px 6px rgba(0,0,0,0.06)" } : {}}>{msg.text}</div>
                <span className="text-[10px] text-gray-400 mx-1">{msg.time}</span>
              </div>
            </div>
      ...
      {chatView === "trip" && (
        <aside className="w-72 flex-shrink-0 bg-white border-l border-orange-50 overflow-y-auto p-5" style={{ scrollbarWidth: "none" }}>
          ...
          <TripAvatar trip={activeTrip} className="relative h-28 rounded-2xl text-3xl mb-4" />
          <div className="space-y-2 mb-5">{[{ icon: <Calendar size={13} className="text-orange-400" />, label: activeTrip.date }, { icon: <Clock size={13} className="text-orange-400" />, label: activeTrip.time }, { icon: <Calendar size={13} className="text-orange-400" />, label: activeTrip.endDate }, { icon: <Clock size={13} className="text-orange-400" />, label: activeTrip.endTime }, { icon: <TransportIcon type={activeTrip.transport} size={13} />, label: transportLabel(activeTrip.transport) }, { icon: <Users size={13} className="text-orange-400" />, label: participantsLabel(totalParticipants) }].map((r, i) => <div key={i} className="flex items-center gap-2.5 text-sm text-gray-700 font-medium">{r.icon}{r.label}</div>)}</div>
          <div className="border-t border-orange-50 pt-4">
            ...
            <div className="space-y-3">
              {[{ name: activeTrip.host.name, avatar: activeTrip.host.avatar, role: "Организатор" }, ...activeTrip.participants.map((p) => ({ name: p.name, avatar: p.avatar, role: "Путешественник" }))].map((member, i) => (
    ...
    <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
      ...
      <div className="px-10 pb-12">
          ...
          <div className="flex-1 pb-2">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex gap-2 mt-3 flex-wrap">{["Ранняя пташка", "Автопутешественник"].map((b) => <span key={b} className="bg-orange-100 text-orange-700 px-3 py-1 rounded-lg text-xs font-bold">{b}</span>)}</div>
              </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          {[{ value: "48", label: "Отзывов", icon: <MessageSquare size={18} className="text-orange-500" /> }, { value: "23", label: "Друзей", icon: <Users size={18} className="text-orange-500" /> }].map((stat, i) => (
  ...
  return (
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {screen === "detail" && (
          <DesktopTripDetail trip={selectedTrip} onBack={() => setScreen("home")} onJoin={() => { setSelectedDm(null); setScreen("chat"); }} />
        )}
        {screen === "chat" && (
          <DesktopChat
            trip={selectedTrip}
            selectedDm={selectedDm}
            onSelectTrip={(t) => { setSelectedTrip(t); setSelectedDm(null); }}
...
export default function App() {
  ...
  const showMobileNav = !["onboarding", "detail", "tripChat", "dmChat"].includes(screen);
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-background overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {screen === "home" && <HomeScreen onTripSelect={(t) => { setSelectedTrip(t); setScreen("detail"); }} />}
        {screen === "detail" && <TripDetailScreen trip={selectedTrip} onBack={() => setScreen("home")} onJoin={() => { setSelectedDm(null); setScreen("chat"); setActiveTab("chat"); }} />}
        {screen === "create" && <CreateTripScreen onBack={() => { setScreen("home"); setActiveTab("home"); }} />}
        {screen === "chat" && <ChatsListScreen onTripChat={(t) => { setSelectedTrip(t); setScreen("tripChat"); }} onDmChat={(dm) => { setSelectedDm(dm); setScreen("dmChat"); }} />}
        {screen === "tripChat" && <ChatScreen trip={selectedTrip} onBack={() => setScreen("chat")} />}
        {screen === "dmChat" && selectedDm && <DmChatScreen dm={selectedDm} onBack={() => { setSelectedDm(null); setScreen("chat"); }} />}
